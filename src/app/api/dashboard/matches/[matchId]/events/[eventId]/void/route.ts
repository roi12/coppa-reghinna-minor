...
import { voidMatchPlayerEventSchema } from "@/features/matches/schemas/update-match-player-event.schema";
import {
...
export async function POST(
  ...
  const parsed = voidMatchPlayerEventSchema.safeParse(payload ?? {});

  if (!parsed.success) {
    return NextResponse.json(
      {
        error: "Invalid void payload.",
  ...
  try {
    const result = await voidMatchPlayerEvent({
      matchId,
      eventId,
      ...parsed.data,
      userId: user.id,
