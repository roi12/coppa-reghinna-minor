...
export async function resolveTournamentKnockoutParticipantsAction(formData: FormData) {
  ...
  try {
    const tournament = await ensureTournamentExists(parsed.data.tournamentId, parsed.data.tournamentSlug);
    const stageDefinitions = mapPersistedStagesToCompetitionInput(tournament.stages);
    const hasGroupStage = stageDefinitions.some((stage) => stage.type === "GROUP_STAGE");
    const hasKnockoutStage = stageDefinitions.some((stage) => stage.type === "KNOCKOUT_STAGE");

    if (!hasGroupStage || !hasKnockoutStage) {
      throw new Error("No preliminary-to-knockout qualification stage is configured for this tournament.");
    }
    const { updateCount } = await resolveTournamentKnockoutParticipants(tournament.id);
